...
export function showToaster(type, title, message) {
  toaster.show(props => {
    ...
    return (
      <Toast toastId={props.toastId} class="toast border border-gray-500 p-4 rounded-md bg-secondary-background flex flex-col items-center justify-between gap-4">
          ...
          <Toast.ProgressTrack class="h-1 w-full rounded-md bg-inactive-background">
            <Toast.ProgressFill
              class={`toast__progress-fill rounded-md h-full ${
                type === 'info' ? 'bg-blue-foreground' :
                type === 'success' ? 'bg-green-foreground' :
                type === 'error' ? 'bg-red-foreground' : 'bg-blue-foreground'
              }`}
            />
          </Toast.ProgressTrack>
        </div>
