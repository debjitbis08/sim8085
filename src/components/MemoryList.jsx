...
export default function MemoryList({ threshold = 4 }) {
    ...
    const deleteCustomRange = (range) => {
        ...
        const updatedCustomRanges = customRanges().filter(
            (customRange) => customRange.start !== range.start || customRange.end !== range.end,
        );
    ...
    const dataRanges = createMemo(() => {
        ...
        const isWithinCustomRange = (range, customRangeList) => {
            return customRangeList.some((customRange) => {
                return range.start >= customRange.start && range.end <= customRange.end;
            });
        };
            ...
            <div class="flex justify-between items-center gap-1 mb-0 mt-2">
                ...
                <Tooltip>
                    <Tooltip.Trigger
                        ...
                        onClick={() => setIsAddingCustom((isAddingCustom) => !isAddingCustom)}
                    >
                        <AiOutlinePlus
                            class={`transition-transform ${isAddingCustom() || editingCustom().start !== null ? "rotate-45" : "rotate-0"}`}
                        />
                    </Tooltip.Trigger>
                    <Tooltip.Portal>
                        <Tooltip.Content class="tooltip__content">
                            ...
                            <Tooltip.Content class="tooltip__content">
                                ...
                                <p>
                                    {inputRange().start === "" || inputRange().end === ""
                                        ? "Please enter both start and end value to watch"
                                        : "Click to add range"}
                                </p>
                            </Tooltip.Content>
                        </Tooltip.Portal>
            ...
            {currentRange().start != null && currentRange().end != null ? (
                <div class="memory-list mt-4 grow min-h-0">
                    <div class="h-full flex flex-col">
                        ...
                        <p class="text-secondary-foreground flex items-center gap-2 my-2">
                            ...
                            <span class="text-xs">
                                Double click the value to edit then press Enter to save the value or Tab to edit the
                                next location.
                            </span>
                        </p>
                ...
                <>
                    ...
                    <p class="text-secondary-foreground text-center mt-8 flex flex-col items-center">
                        ...
                        <span>
                            This section allows to watch memory ranges you are interested in and also edit them.
                        </span>
                    </p>
                </>
            )}
