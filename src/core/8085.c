...
int Emulate8085Op(State8085* state)
{
	int cycles = 4;
	...
	switch (state->memory[state->pc - 1])
	{
		...
		case 0x07:                          //RLC
            ...
            break;
		case 0x08: InvalidInstruction(state); break;
		case 0x09: 							// DAD B
			{
		...
		case 0x0f: 							//RRC
			...
			break;
		case 0x10: InvalidInstruction(state); break;
		...
		case 0x17:                          //RAL
            ...
            break;
		case 0x18: InvalidInstruction(state); break;
		case 0x19: 							//DAD D
			{
			uint32_t hl = (state->h << 8) | state->l;
		...
		case 0x27:                                              // DAA
		    ...
		    break;
		case 0x28: InvalidInstruction(state); break;
		case 0x29: 								                // DAD H
			{
			uint32_t hl = (state->h << 8) | state->l;
			uint32_t res = hl + hl;
		...
		case 0x38: InvalidInstruction(state); break;
		case 0x39:                                              // DAD SP
			{
		...
		case 0x3e: 							                    // MVI A, byte
			...
			break;
		case 0x3f:                                              // CMC
		    if (0 == state->cc.cy) state->cc.cy = 1;
		    else state->cc.cy = 0;
		    break;
		...
		case 0x51: state->d = state->c; break;                  // MOV D, B
		case 0x52: state->d = state->d; break;                  // MOV D, B
		case 0x53: state->d = state->e; break;                  // MOV D, B
		case 0x54: state->d = state->h; break;                  // MOV D, B
		case 0x55: state->d = state->l; break;                  // MOV D, B
		case 0x56: 							                    // MOV D, M
			{
                uint16_t offset = (state->h<<8) | (state->l);
		...
		case 0xa7:                                   // ANA A
		    ...
		    break;
		case 0xa8:
		    state->a = state->a ^ state->b; LogicFlagsA(state, 0);	break; // XRA B
		case 0xa9:
		    state->a = state->a ^ state->c; LogicFlagsA(state, 0);	break; // XRA C
		case 0xaa:
		    state->a = state->a ^ state->d; LogicFlagsA(state, 0);	break; // XRA D
		case 0xab:
		    state->a = state->a ^ state->e; LogicFlagsA(state, 0);	break; // XRA E
		case 0xac:
		    state->a = state->a ^ state->h; LogicFlagsA(state, 0);	break; // XRA H
		case 0xad:
		    state->a = state->a ^ state->l; LogicFlagsA(state, 0);	break; // XRA L
		case 0xae:                                                     // XRA M
		    {
			uint16_t offset = (state->h<<8) | (state->l);
		    state->a = state->a ^ state->memory[offset];
		    ...
		    break;
		case 0xaf:
		    state->a = state->a ^ state->a; LogicFlagsA(state, 0);	break; // XRA A
		case 0xb0:
		    state->a = state->a | state->b; LogicFlagsA(state, 0);	break; // ORA B
		case 0xb1:
		    state->a = state->a | state->c; LogicFlagsA(state, 0);	break; // ORA C
		case 0xb2:
		    state->a = state->a | state->d; LogicFlagsA(state, 0);	break; // ORA D
		case 0xb3:
		    state->a = state->a | state->e; LogicFlagsA(state, 0);	break; // ORA E
		case 0xb4:
		    state->a = state->a | state->h; LogicFlagsA(state, 0);	break; // ORA H
		case 0xb5:
		    state->a = state->a | state->l; LogicFlagsA(state, 0);	break; // ORA L
		case 0xb6:                                                     // ORA M
		    {
			uint16_t offset = (state->h<<8) | (state->l);
		    state->a = state->a | state->memory[offset];
		    ...
		    break;
		case 0xb7:
		    state->a = state->a | state->a; LogicFlagsA(state, 0);	break; // ORA A
		case 0xb8:                                                     // CMP B
		    subtractByte(state, state->a, state->b); break;
		case 0xb9:                                                     // CMP C
		    subtractByte(state, state->a, state->c); break;
		case 0xba:                                                     // CMP D
		    subtractByte(state, state->a, state->d); break;
		case 0xbb:                                                     // CMP E
		    subtractByte(state, state->a, state->e); break;
		case 0xbc:                                                     // CMP H
		    subtractByte(state, state->a, state->h); break;
		case 0xbd:                                                     // CMP L
		    subtractByte(state, state->a, state->l); break;
		case 0xbe:                                                     // CMP M
		    {
			uint16_t offset = (state->h<<8) | (state->l);
		    ...
		    break;
		case 0xbf:                                                     // CMP A
		    subtractByte(state, state->a, state->a); break;
		case 0xc0:                                                     // RNZ
		    if (0 == state->cc.z) returnToCaller(state);
			break;
		case 0xc1: 						                               // POP B
			{
		...
		case 0xc2: 						                               // JNZ Addr
			...
			break;
		case 0xc3:						                               // JMP Addr
			state->pc = (state->pc - 1) + ((opcode[2] << 8) | opcode[1]);
			break;
		case 0xc4:                                                     // CNZ Addr
			if (0 == state->cc.z)
			{
		...
		case 0xc8:                                                     // RZ
		    if (1 == state->cc.z) returnToCaller(state);
			break;
		case 0xc9: 						                                // RET
		    returnToCaller(state); break;
		case 0xca:                                                      // JZ Addr
		    if (1 == state->cc.z)
		        state->pc = (opcode[2] << 8) | opcode[1];
			else
				state->pc += 2;
			break;
		case 0xcb: InvalidInstruction(state); break;
		case 0xcc:                                                      // CZ Addr
			if (1 == state->cc.z)
		...
		case 0xcf:                                                      // RST 1
		    UnimplementedInstruction(state); break;
		case 0xd0:                                                      // RNC
		    if (0 == state->cc.cy) returnToCaller(state);
			break;
		case 0xd1: 						                                // POP D
			{
			...
			break;
		case 0xd2:
		    UnimplementedInstruction(state); break;              // JNC or INC
		case 0xd3:
		...
		case 0xd7:                                                       // RST 2
		    UnimplementedInstruction(state); break;
		case 0xd8:                                                       // RC
		    if (1 == state->cc.cy) returnToCaller(state);
			break;
		case 0xd9: InvalidInstruction(state); break;
		case 0xda:                                                      // JC Addr
		    if (1 == state->cc.cy)
		        state->pc = (opcode[2] << 8) | opcode[1];
			else
				state->pc += 2;
			break;
		case 0xdb: UnimplementedInstruction(state); break;              // IN d8
		case 0xdc:                                                      // CC Addr
		    if (1 == state->cc.cy)
		        call(state, (opcode[2] << 8) | opcode[1]);
            else
		        state->pc += 2;
		    break;
		case 0xdd: InvalidInstruction(state); break;
		case 0xde:                                                      // SBI d8
		...
		case 0xec:                                                      // CPE Addr
		    if (1 == state->cc.p)
		        ...
		        state->pc += 2;
		    break;
		case 0xed: InvalidInstruction(state); break;
		case 0xee:                                                      // XRI d8
		    state->a = state->a ^ opcode[1];
		    LogicFlagsA(state, 0);
		    state->pc++;
		...
		case 0xef:                                                      // RST 5
		    UnimplementedInstruction(state); break;
		case 0xf0:                                                      // RP
		    if (0 == state->cc.s) returnToCaller(state);
			break;
		case 0xf1: 					//POP PSW
			{
		...
		case 0xf7:                                                      // RST 6
		    UnimplementedInstruction(state); break;
		case 0xf8:                                                      // RM
		    if (1 == state->cc.s) returnToCaller(state);
			break;
		case 0xf9:                                                      // SPHL
		...
		case 0xfc:                                                      // CM Addr
		    if (1 == state->cc.s)
		        ...
		        state->pc += 2;
		    break;
		case 0xfd: InvalidInstruction(state); break;
		case 0xfe: 						                                // CPI d8
			{
